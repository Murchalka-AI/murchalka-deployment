...
4. Start Runtime with installation `minimal-core`; Compose first verifies nested rootless Bubblewrap support with the one-shot `sandbox-probe` service. Runtime applies `bindings/minimal.bindings.yaml` after providers are installed. The logical provider instance is `default`.
...
If `sandbox-probe` fails, inspect its Compose log before troubleshooting modules. Runtime and the probe deliberately run as non-root with all outer Linux capabilities dropped; the Docker host must support unprivileged user namespaces and the Compose security options declared for those two services.
