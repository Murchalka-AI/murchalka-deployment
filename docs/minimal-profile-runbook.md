...
4. Start Runtime with installation `minimal-core`; Compose first verifies the capability-free `unshare` → Bubblewrap sandbox path with the one-shot `sandbox-probe` service. Runtime applies `bindings/minimal.bindings.yaml` after providers are installed. The logical provider instance is `default`.
