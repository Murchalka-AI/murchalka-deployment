...
Phase 7 adds the signed Client Extension delivery gate, the `releases/client-runtime.lock.json` coordinated component set, and `scripts/phase7-e2e.sh`. The gate starts an already-built Runtime, drops an already-signed diagnostics bundle into its inbox, verifies the live content-addressed catalog and server action, then proves immediate disable and re-enable revision propagation. Web and Desktop run the same browser acceptance for signed WASM, fallback, corrupt/unsigned rejection, activation rollback, and verified offline cache reuse.
