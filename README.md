...
Phase 8 adds the generic Protocol Gateway and independently signed MCP and A2A modules. `scripts/phase8-e2e.sh` starts a released Runtime, hot-installs the protocol bundles, exercises a CI-signed conformance provider through allowed MCP and A2A mappings, proves progress and downstream cancellation, checks denied activity in the redacted Root audit, invalidates sessions on route revocation, quarantines unsigned and corrupt bundles, rejects an incompatible bundle, verifies activation rollback, and finally removes the listener. The cross-platform declarative UI catalog and status actions are checked against the same live state. The coordinated set is pinned in `releases/protocol-modules.lock.json`; deployment publication requires the `Phase 8 E2E` reusable workflow.
